...
# %% auto 0
__all__ = ['pp', 'repo', 'get_argparser', 'npa_to_packed_buffer', 'flash_xcp', 'downlod_calib_data']
...
def downlod_calib_data(xcp_calib: XCPCalib, diff_flashing: bool=False):
    ...
    for d in xcp_calib.data:
        ...
        tiles = len_in_bytes // 6
        last_tile = len_in_bytes % 6
        # Download full size tiles with DNLOAD_6
        for i in range(tiles):
            ctr += 1
            cro = CCP(identifier=xcp_calib.config.download_can_id)/CRO(ctr=ctr)/DNLOAD_6(data=raw(d.value_array_view[i*6:(i+1)*6]))
            sock.send(cro)
            rx_cro = sock.recv()
        start_index = tiles * 6
        ctr += 1 
        cro = CCP(identifier=xcp_calib.config.download_can_id)/CRO(ctr=ctr)/DNLOAD(data=raw(d.value_array_view[start_index:start_index+last_tile]))
        sock.send(cro)
        rx_cro = sock.recv()
    sock.close()
