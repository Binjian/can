# AUTOGENERATED! DO NOT EDIT! File to edit: ../../nbs/02.ccp.virtual.ipynb.
...
# %% ../../nbs/02.ccp.virtual.ipynb 7
import pandas as pd
import numpy as np
import struct

# %% ../../nbs/02.ccp.virtual.ipynb 9
...
def downlod_calib_data(
    ...
    match can_type:
        case "NATIVE":
            ...
            match bus_type:
                ...
                case _:
                    raise ValueError(
                        f"Invalid CAN bus type: {bus_type}, valid types are: SOCKET or VIRTUAL for Native CANSOCKET"
                    )
        case "PYTHON":
            ...
            match bus_type:
                ...
                case _:
                    raise ValueError(
                        f"Invalid CAN bus type: {bus_type}, implemented valid types are: SOCKET, KVASER, VECTOR for Python-CAN CANSOCKET"
                    )

    # CONNECT
    ctr += 1
    cro = (
        CCP(identifier=xcp_calib.config.download_can_id)
        / CRO(ctr=ctr)
        / CONNECT(station_address=0x00)
    )
    ...
    for d in xcp_calib.data:
        ...
        cro = (
            CCP(identifier=xcp_calib.config.download_can_id)
            / CRO(ctr=ctr)
            / SET_MTA(address=int(d.address, 16))
        )
        dto = sock.sr1(cro, timeout=timeout)
        assert dto.return_code == 0x00
        ...
        for i in range(tiles):
            ...
            cro = (
                CCP(identifier=xcp_calib.config.download_can_id)
                / CRO(ctr=ctr)
                / DNLOAD_6(data=d.value_bytes[i * tile_size : (i + 1) * tile_size])
            )
            dto = sock.sr1(cro, timeout=timeout)
            assert dto.return_code == 0x00
        ...
        cro = (
            CCP(identifier=xcp_calib.config.download_can_id)
            / CRO(ctr=ctr)
            / DNLOAD(data=d.value_bytes[start_index : start_index + last_tile])
        )
        dto = sock.sr1(cro, timeout=timeout)
        assert dto.return_code == 0x00
    ...
    cro = (
        CCP(identifier=xcp_calib.config.download_can_id)
        / CRO(ctr=ctr)
        / DISCONNECT(station_address=0x00)
    )
    dto = sock.sr1(cro, timeout=timeout)
    assert dto.return_code == 0x00
